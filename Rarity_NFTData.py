...
class Rarity_NFTData:
    ...
    def display_pretty(self):
        ...
        print("Release Price: {}".format(self.release_price))
        ...
        print("Discord: {}".format(self.discord_id))
        print("Twitter: {}".format(self.twitter_id))
    ...
    def asDict(self):
        ...
        d['twitter_id'] = self.twitter_id
        d['discord_id'] = self.discord_id
    ...
    def extract_price_info(self, text):
        ...
        if len(lines)==1:
            self.release_price, self.currency = self.parse_price(lines[0])
            self.presale_price = None
        elif len(lines)>1:
            if len(lines)>2:
    ...
    def parse_release_date(self, s):
        ...
        result_as_string = result.strftime('%Y-%m-%d')
    ...
    def import_from_rarity_page_element(self, listing):
        ...
        if listing.discordurl:
            self.discord_id = self.discord_user_from_url(listing.discordurl)
        if listing.twitterurl:
            self.twitter_id = self.twitter_user_from_url(listing.twitterurl)
